...
The current approved production build predates this GitHub repository and lives in the canonical AWS Git tree at `/opt/openflux/src/openflux-yandex`.
...
AWS remains the approval point. Mini-PCs must not independently follow upstream.

## Repository bootstrap rule

The exact AWS canonical source tree is authoritative until it is imported here. Do not reconstruct production source from memory or from partial chat excerpts if the AWS tree is available.
